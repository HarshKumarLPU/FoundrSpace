...
<body class="bg-slate-950 text-slate-100 antialiased selection:bg-slate-100 selection:text-slate-950 flex flex-col md:flex-row min-h-screen" x-data="{ mobileMenuOpen: false }">
    ...
    <aside class="hidden md:flex flex-col w-72 bg-slate-900 border-r border-slate-800/80 h-screen sticky top-0 left-0 z-30 flex-shrink-0 justify-between">
        <div class="flex flex-col flex-grow">
            ...
            <!-- Navigation Links -->
            <nav class="flex-grow px-4 py-8 space-y-1.5">
        ...
        <div class="p-4 border-t border-slate-800/60 bg-slate-900/40">
            @auth
                <a href="{{ url('/dashboard') }}" class="w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white font-semibold text-sm rounded-xl border border-indigo-500 transition-colors shadow-sm">
    ...
    <div x-show="mobileMenuOpen" class="md:hidden fixed inset-0 z-35 flex bg-slate-950/80 backdrop-blur-sm" style="display: none;">
        <div class="w-64 bg-slate-900 border-r border-slate-800 h-full flex flex-col justify-between pt-20 pb-6 px-4 animate-slide-in">
            ...
            <div class="border-t border-slate-800/60 pt-4 flex flex-col gap-2">
                @auth
                    <a @click="mobileMenuOpen = false" href="{{ url('/dashboard') }}" class="w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white font-semibold text-sm rounded-xl border border-indigo-500 transition-colors">Dashboard</a>
                @else
                    <a @click="mobileMenuOpen = false" href="{{ route('login') }}" class="w-full flex items-center justify-center py-2.5 text-sm font-semibold text-slate-400 hover:text-slate-100 transition-colors">Log in</a>
