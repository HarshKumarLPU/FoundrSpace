            ...
            <div class="bg-gray-800/50 backdrop-blur-xl overflow-hidden shadow-2xl sm:rounded-2xl border border-gray-700">
                <div class="p-8 text-gray-100">
                    <form method="POST" action="{{ route('startups.store') }}" enctype="multipart/form-data" class="space-y-6">
                        ...
                        <!-- Logo -->
                        <div>
                            <x-input-label for="logo" :value="__('Startup Logo')" class="text-gray-300" />
                            <input id="logo" type="file" name="logo" class="block mt-1 w-full text-sm text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-cyan-500/10 file:text-cyan-400 hover:file:bg-cyan-500/20" />
                            <x-input-error :messages="$errors->get('logo')" class="mt-2" />
                        </div>
                        
                        <!-- Banner -->
                        <div>
                            <x-input-label for="banner" :value="__('Cover Banner')" class="text-gray-300" />
